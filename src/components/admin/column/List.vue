        ...
        <el-table-column
          ...
          label="操作">
            <template slot-scope="scope">
              ...
              <p>
                <router-link :to="{name: 'adminColumnEdit', params: {id: scope.row.id}}">编辑</router-link>
...
export default {
  ...
  methods: {
    ...
    sureDialogSubmit () {
      ...
      self.$http.delete(api.adminColumn, {params: {id: self.currentDialogId}})
    ...
    setStatus(index, item, evt) {
      var id = item.id
      var self = this
      self.$http.put(api.adminColumnChangeStatus, {id: id, status: evt})
      .then (function(response) {
        if (response.data.meta.status_code === 200) {
    ...
    loadList() {
      ...
      self.isLoading = true
      self.$http.get(api.adminColumnList, {params: {page: self.query.page, per_page: self.query.pageSize, sort: self.query.sort, type: self.query.type, status: self.query.status}})
      .then (function(response) {
