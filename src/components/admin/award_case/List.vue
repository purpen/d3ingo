            ...
            <el-table-column
              ...
              label="操作">
                <template slot-scope="scope">
                  ...
                  <p>
                    <router-link :to="{name: 'adminAwardCaseEdit', params: {id: scope.row.id}}">编辑</router-link>
...
export default {
  ...
  methods: {
    ...
    sureDialogSubmit () {
      ...
      self.$http.delete(api.adminAwardCase, {params: {id: self.currentDialogId}})
    ...
    // 状态设置
    setRecommended(index, item, evt) {
      var id = item.id
      var self = this
      self.$http.put(api.adminAwardCaseChangeRecommended, {id: id, evt: evt})
      .then (function(response) {
        if (response.data.meta.status_code === 200) {
          self.tableData[index].recommended = evt
    ...
    setStatus(index, item, evt) {
      var id = item.id
      var self = this
      self.$http.put(api.adminAwardCaseChangeStatus, {id: id, status: evt})
      .then (function(response) {
        if (response.data.meta.status_code === 200) {
