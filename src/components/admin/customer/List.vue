        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <div v-if="scope.row.new_source || scope.row.new_source === 0" class="fz-14 tc-3">
              <p>{{scope.row.new_source | promoteSource}}</p>
              <span>{{scope.row.son_source | promoteSonSource(scope.row.new_source)}}</span>
            </div>
            <div v-else>
...
import { PROMOTE_SOURCE } from '@/config'
