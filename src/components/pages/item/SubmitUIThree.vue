              ...
              <el-form-item label="已有项目设计内容" prop="complete_content">
                <el-checkbox-group v-model="form.complete_content">
                  ...
                  <el-checkbox label="其它" @change="otherContent">其它
                    <el-input v-show="showOtherContent" v-model="form.other_content" size="small"
                              placeholder="自定义添加"></el-input>
                  </el-checkbox>
                </el-checkbox-group>
              </el-form-item>
          ...
          <div id="right_box" :class="{'fixed': isFixed}">
            <div class="slider" v-loading="matchLoading">
              ...
              <div v-if="matchCount === 0">
                ...
                <p class="slide-des error">当前项目设计周期太短，无法匹配有效的设计服务供应商，请重新设置交付时间。</p>
...
<script>
  ...
  export default {
    ...
    methods: {
      montiorWindow() {
        ...
        if (currentScroll > this.scroll) {
          let rObj = document.querySelector('#right_box')
          let l1 = document.querySelector('.container').offsetLeft
          let l2 = document.querySelector('.content').offsetWidth
          let w = rObj.offsetWidth
    ...
    computed: {
      ...
      cycleOptions() {
        let items = []
        for (let i = 0; i < typeData.CYCLE_OPTIONS.length; i++) {
          let item = {
      ...
      costOptions() {
        let items = []
        for (let i = 0; i < typeData.DESIGN_COST_OPTIONS.length; i++) {
          let item = {
...
//              console.log(row)
              ...
              if (row.competing_product) {
                for (let i = 0; i < row.competing_product.length; i++) {
                  let val = {value: row.competing_product[i]}
                  that.form.cProducts[i] = val
                }
              }
              if (response.data.data.item.image) {
                let files = []
                for (let i = 0; i < response.data.data.item.image.length; i++) {
                  if (i > 5) {
                    break
                  }
                  let obj = response.data.data.item.image[i]
                  let item = {}
                  item['response'] = {}
                  item['name'] = obj['name']
                  item['url'] = obj['small']
