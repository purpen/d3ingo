<template>
  <section :class="['menu-sub','project-menu-sub', {'jdc': custom.name === 'jdc', 'yw': custom.name === 'yw'}]">
    <div class="nav-left nav-menu">
      <router-link :to="{name: 'home'}" class="el-menu-item logo" :class="{'sn': custom.name === 'sn' || custom.name === 'jdy_jn'}">
    ...
    <div class="nav-right nav-menu" v-if="token">
      <el-menu class="el-menu-info" mode="horizontal" router v-if="custom.id === 0">
        <el-submenu index="2" :popper-append-to-body="false">
          <template slot="title">
            ...
            <span v-if="user.company && (user.company.company_abbreviation || user.company.company_name)" class="b-nickname">{{ user.company.company_abbreviation || user.company.company_name }}</span>
      ...
      <el-menu class="el-menu-info" mode="horizontal" router v-else>
        <el-submenu index="2">
          ...
          <el-menu-item index="/b_admin/item/list" v-if="isSysAdmin || (user.source_admin === 1 && custom.name === 'jdc') || (user.source_admin === 2 && custom.name === 'yw') || (user.source_admin === 4 && custom.name === 'sn') || (user.source_admin === 7 && custom.name === 'jdy_jn') || user.source_admin === 6"><i class="fx-4 fx-icon-control-center"></i><i class="fx-4 fx-icon-console-hover"></i>后台管理</el-menu-item>
          <el-menu-item index="" @click="logout"><i class="fx-4 fx-icon-logout"></i><i class="fx-4 fx-icon-logout-hover"></i>安全退出</el-menu-item></el-submenu>
      </el-menu>
    </div>
